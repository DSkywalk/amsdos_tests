...
if sys.argv.__len__() != 4:
    print('Error: invalid args\nusage:', sys.argv[0], '<source-bas-path> <new-length> <dest-bas-path>')
    exit(1)

current_file = sys.argv[1]
file_size = -1
new_length = int(sys.argv[2], 10)
new_file = sys.argv[3]
...
with open(current_file, 'rb') as lr:
    ...
    # get file size, os.stat is for lusers :P
    lr.seek(0, os.SEEK_END)
    file_size = lr.tell() 

    # copy to new file
    fp2 = open(new_file,'wb')
    lr.seek(0, os.SEEK_SET)
    data = lr.read(file_size)
    fp2.write(data)

    # change length
    fp2.seek(24)
    # logical_length
    print('old_length', hex(logical_length), '=> new_length', hex(new_length))
    fp2.write(struct.pack("<H", new_length))
    # file_length
    fp2.seek(36 + 2, os.SEEK_CUR) # pass 36 + 2 bytes
    fp2.write(struct.pack("<H", new_length))
    fp2.close()
